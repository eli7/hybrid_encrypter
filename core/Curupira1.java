...
public class Curupira1 implements BlockCipher {
     ...
     * 
     * @param u
     *            a byte representing a polynomial.
     * @return a byte representing u * 0x02 (i.e. x).
     */
     ...
     * @param u
     *            a byte.
     * @return a byte representing the polynomial u * c(x).
     */
     ...
     *            Resulting 3x4 matrix.
     * @param e
     *            pass true for using E or false for using the inverse of E.
     */
     ...
     *            3x4 matrix.
     * @param kr
     *            3x4 matrix representing sub-key.
     * @return Resulting 3x4 matrix.
     */
    ...
    /**
     * Performs a Curupira-1 whitening round on matrix a.
     * 
     * @param a
     *            3x4 matrix (from previous round).
     * @param k0
     *            k(0) round key.
     * @return Resulting 3x4 matrix.
     */
    ...
    /**
     * Performs a Curupira-1 last round on matrix a.
     * 
     * @param a
     *            3x4 matrix (from previous round).
     * @param kR
	...
	// see page 9.
	return applyKeyAddition(applyPermutationLayer(applyNonLinearLayer(a)),
		kR);
    }

    /**
     * Performs a Curupira-1 "intermediary" round on matrix a.
     * 
     * @param a
     *            3x4 matrix (from previous round).
     * @param kr
	...
	return applyKeyAddition(
		applyLinearDiffusionLayer(applyPermutationLayer(applyNonLinearLayer(a))),
		kr);
    }

    /**
     * Performs a Curupira-1 unkeyed round on matrix a.
     * 
     * @param a
     *            3x4 matrix (from previous round).
     * @return Resulting 3x4 matrix.
    ...
    /**
     * Applies constant addition to sub-key Kr.
     * 
     * @param Kr
     *            3x2t matrix representing the Kr sub-key.
     * @param subkeyRank
     *            Sub-key rank.
     * 
    ...
    /**
     * Gets sub-key for the current round given sub-key from the previous round.
     * 
     * @param kr
     *            3x4 matrix representing the Kr sub-key.
     * @return 3x4 matrix representing the Kr+1 sub-key.
     */
    public byte[] calculateNextSubkey(byte[] Kr, int subkeyRank) {
	// see pages 7, 8 and 9.
	return applyLinearDiffusion(applyCyclicShift(applyConstantAddition(Kr,
		subkeyRank)));
    }

    /**
     * Selects the round key that corresponds to the given sub-key.
     * 
     * @param kr
     *            3x4 matrix representing the Kr sub-key.
	...
	// see pages 9 and 10.
	encryptionRoundKeys = new byte[R + 1][12];
	decryptionRoundKeys = new byte[R + 1][12];
	byte[] Kr = cipherKey;
	byte[] kr = selectRoundKey(Kr);
	encryptionRoundKeys[0] = kr;
	for (int r = 1; r <= R; r++) {
	    Kr = calculateNextSubkey(Kr, r);
	    kr = selectRoundKey(Kr);
	    encryptionRoundKeys[r] = kr;
	    decryptionRoundKeys[R - r] = applyLinearDiffusionLayer(kr);
	}
	decryptionRoundKeys[0] = encryptionRoundKeys[R];
	decryptionRoundKeys[R] = encryptionRoundKeys[0];
    }

    // ------------------- CURUPIRA ENCRYPTION/DECRYPTION --------------------
	...
	cBlockTmp = performLastRound(cBlockTmp, roundKeys[R]);
    ...
    /**
     * This is set through the makeKey method; Curupira accepts keys of size 96,
     * 144 and 192 bits.
     */
    public int keyBits() {
	return keyBits;
     ...
     * 
     * @param cipherKey
     *            the cipher key of keyBits size. It must be serialized in
     *            column major order.
     * @param keyBits
     *            size of the cipher key in bits. This must be either 96, 144 or
     *            192.
     */
	...
	// see page 9.
	processBlock(mBlock, cBlock, encryptionRoundKeys);
    }
	...
	// see page 10.
	processBlock(cBlock, mBlock, decryptionRoundKeys);
    }
